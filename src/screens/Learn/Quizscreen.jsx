    ...
    <div style={{ width: "100%", maxWidth: 390, background: "#fff", minHeight: "100vh", display: "flex", flexDirection: "column", boxShadow: "0 0 60px rgba(0,0,0,0.1)" }}>
      ...
      <div style={{ padding: "52px 20px 16px", background: "#fff", borderBottom: "1px solid #F1F5F9" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
          <button onClick={() => navigate("interactiveDiagram")} style={{ width: 38, height: 38, borderRadius: 12, background: "#F1F5F9", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <ArrowLeft size={18} color="#374151" strokeWidth={2.5} />
          </button>
          <div>
            <h1 style={{ fontSize: 17, fontWeight: 800, color: "#0F172A", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Module Quiz</h1>
            <p style={{ fontSize: 12, color: "#64748B", fontWeight: 500 }}>Test your understanding</p>
          </div>
        </div>

        {/* Progress */}
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#64748B", letterSpacing: "0.08em", textTransform: "uppercase" }}>Quiz Progress</p>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#64748B", letterSpacing: "0.08em", textTransform: "uppercase" }}>Question {current + 1} / {questions.length}</p>
        </div>
      ...
      {/* Body */}
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px 24px" }} key={current}>

        {/* Question card */}
        <div className="qz-slide" style={{ background: "#F8FAFF", borderRadius: 18, padding: "20px", marginBottom: 20, border: "1px solid #E0EAFF" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#2563EB", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Question {current + 1} of {questions.length}</p>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "#0F172A", lineHeight: 1.4, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{q.question}</h2>
        </div>
          ...
          {q.options.map((opt, idx) => {
            ...
            return (
              <button key={idx} className="qz-opt" onClick={() => handleSelect(idx)}
                style={{ background: bg, border: `1.5px solid ${border}`, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 13, boxShadow: isSelected ? "0 4px 16px rgba(37,99,235,0.12)" : "0 1px 6px rgba(0,0,0,0.04)", width: "100%" }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: labelBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s ease" }}>
                  {submitted && isCorrect ? <CheckCircle size={18} color="#fff" strokeWidth={2.5} /> :
                    <span style={{ fontSize: 13, fontWeight: 800, color: labelColor, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{LABELS[idx]}</span>}
                </div>
                <span style={{ fontSize: 14, fontWeight: isSelected ? 700 : 500, color: textColor, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{opt}</span>
              </button>
            );
          })}
        </div>
      ...
      <div style={{ padding: "12px 16px 36px", background: "#fff", borderTop: "1px solid #F1F5F9" }}>
        <button className="qz-submit" onClick={handleSubmit} disabled={selected === null || submitted}
          style={{ width: "100%", padding: "15px", borderRadius: 14, color: "#fff", fontSize: 15, fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
