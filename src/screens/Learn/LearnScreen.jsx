...
export default function LearnScreen({ navigate }) {
  return (
    <div style={{ width: "100%", maxWidth: 390, background: "#fff", minHeight: "100vh", display: "flex", flexDirection: "column", boxShadow: "0 0 60px rgba(0,0,0,0.1)" }}>
      ...
      {/* Header */}
      <div style={{ padding: "52px 20px 20px", background: "linear-gradient(160deg, #EFF6FF 0%, #fff 80%)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: "#0F172A", fontFamily: "'Plus Jakarta Sans', sans-serif", lineHeight: 1.2 }}>Learn ECG</h1>
            <p style={{ fontSize: 13, color: "#64748B", marginTop: 3, fontWeight: 500 }}>Study ECG concepts step-by-step</p>
          </div>
          <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg, #BFDBFE, #3B82F6)", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid #DBEAFE" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 16px 32px" }}>
        ...
        <div style={{ marginBottom: 24, marginTop: 4 }}>
          <p style={{ fontSize: 16, fontWeight: 700, color: "#0F172A", marginBottom: 12, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Continue Learning</p>
          <div className="ls-fade" style={{ animationDelay: "0.05s", background: "#fff", borderRadius: 18, padding: "18px 20px", boxShadow: "0 2px 16px rgba(0,0,0,0.07)", border: "1px solid #E2E8F0" }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: "#64748B", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>Chapter 3 of 8</p>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "#0F172A", marginBottom: 16, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>ECG Leads &amp; Axis Determination</h2>
            <button className="ls-resume-btn" onClick={() => navigate("lessonDetail")} style={{ width: "100%", padding: "14px", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: "0.02em" }}>
              Resume Module
            </button>
          </div>
        </div>
        ...
        {/* Modules */}
        <p style={{ fontSize: 16, fontWeight: 700, color: "#0F172A", marginBottom: 12, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Learning Modules</p>
          ...
          {modules.map((mod, i) => {
            ...
            return (
              <div
                ...
                style={{ animationDelay: `${0.06 * i + 0.1}s`, background: "#fff", borderRadius: 16, padding: "14px 16px", boxShadow: "0 1px 8px rgba(0,0,0,0.05)", border: "1px solid #F1F5F9", display: "flex", alignItems: "center", gap: 13, opacity: mod.status === "locked" ? 0.75 : 1 }}
                onClick={() => mod.status !== "locked" && navigate("lessonDetail")}
              >
                <div style={{ width: 44, height: 44, borderRadius: 13, background: mod.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Icon size={20} color={mod.color} strokeWidth={2} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#0F172A", marginBottom: 2, fontFamily: "'Plus Jakarta Sans', sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{mod.title}</p>
                  <p style={{ fontSize: 11, color: "#64748B", fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{mod.desc}</p>
                  {mod.status === "active" && (
                ...
                <div style={{ flexShrink: 0 }}>
                  {mod.status === "done" && <CheckCircle size={20} color="#22C55E" strokeWidth={2} />}
                  {mod.status === "active" && (
                    <div style={{ background: "#EFF6FF", borderRadius: 20, padding: "3px 9px" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: "#2563EB" }}>{mod.progress}%</span>
                    </div>
                  )}
                  {mod.status === "locked" && <Lock size={16} color="#CBD5E1" strokeWidth={2} />}
                </div>
              </div>
            );
          })}
